      ...
      {rows.map((row) => (
        <article key={row.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span
                  className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${statusClassName(
              ...
              <h3 className="text-base font-semibold text-slate-900">{contactDisplay(row)}</h3>
          ...
          <dl className="mt-4 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
            ...
            {row.formula ? (
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">Formule</dt>
                <dd>{row.formula}</dd>
              </div>
            ) : null}
            {row.atout_france ? (
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">Atout France</dt>
                <dd>{row.atout_france}</dd>
              </div>
            ) : null}
            {row.sdjes ? (
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">SDJES</dt>
                <dd>{row.sdjes}</dd>
              </div>
            ) : null}
            {row.website_url ? (
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 whitespace-pre-wrap rounded-xl bg-slate-50 px-3 py-2 text-sm text-slate-700">
              {row.message}
            </p>
          </div>
        </article>
