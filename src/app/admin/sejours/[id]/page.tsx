...
export default async function AdminStayDetailPage({ params }: PageProps) {
  const { id: stayId } = await params;
  await requireRole('ADMIN');
  const supabase = getServerSupabaseClient();

  const { data: stay } = await supabase.from('stays').select('*').eq('id', stayId).maybeSingle();
  ...
  const [
    ...
    supabase
      ...
      .eq('organizer_id', stay.organizer_id)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="admin-page-title">Modifier le séjour</h1>
          <p className="admin-page-subtitle mt-1">
            Tunnel d&apos;édition unifié avec l&apos;espace organisateur.
          </p>
        </div>
        <Link
          href={backHref}
