...
import {
  listDepositReminderCandidates,
  sendDepositReminder
} from '@/lib/deposit-payment-reminder.server';
...
export async function GET(request: Request) {
  ...
  let candidates;
  try {
    candidates = await listDepositReminderCandidates(supabase, { limit });
  } catch (error) {
    console.error('[api/cron/deposit-payment-reminder] list failure', error);
  ...
  for (const candidate of candidates) {
    try {
      const result = await sendDepositReminder(supabase, candidate, { toOverride: toOverride || null });
