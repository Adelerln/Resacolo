...
export default async function MnemosChatbotPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold text-white">Assistant en ligne</h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          { label: 'Clics sur les citations', value: citationClicks },
          { label: 'Transferts vers l’équipe', value: handoffs }
        ].map((metric) => (
          <article
            key={metric.label}
      ...
      <section className="rounded-2xl border border-violet-900/50 bg-slate-900/70 p-4">
        <h2 className="text-lg font-semibold text-white">Taux de clics vers les fiches</h2>
        <p className="mt-1 text-sm text-slate-400">
          Clics sur les citations / réponses affichées :{' '}
          <strong className="text-violet-200">{formatPercent(ctr)}</strong>
        </p>
      </section>

      <section className="rounded-2xl border border-violet-900/50 bg-slate-900/70 p-4">
        <h2 className="text-lg font-semibold text-white">Pages les plus cliquées</h2>
        {topUrls.length === 0 ? (
