                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-3">
                        <button
                          ...
                          aria-controls={`stay-stocks-${stay.id}`}
                          aria-label={`Modifier les stocks de ${stay.title}`}
                          onClick={() => {
                            setOpenStayIds((current) => current.includes(stay.id) ? current : [...current, stay.id]);
                            if (!editingSessionId || !dirtySessionIds[editingSessionId]) {
                              setEditingSessionId(stay.sessions[0]?.id ?? null);
                            }
                          }}
                          className="organizer-btn-secondary min-h-[36px] gap-1 px-3 py-1 text-xs"
                        >
                  ...
                  {isOpen && (
                    <tr key={`${stay.id}-sessions`} className="border-t border-slate-100 bg-slate-100/80">
                      <td colSpan={7} className="px-4 py-4">
                        <div id={`stay-stocks-${stay.id}`} className="rounded-xl border border-slate-200 bg-slate-50">
                          <div className="border-b border-slate-100 px-4 py-3 text-sm font-medium text-slate-900">
                            Stocks par session
                            <p className="mt-1 text-xs font-normal text-slate-500">
                              Renseignez les places encore disponibles à la vente. Les réservations existantes sont conservées.
                            </p>
                          </div>
