...
export function draftReviewSectionClass(v: DraftReviewSectionVariant): string {
  ...
  if (v.hasError) {
    return cn(base, 'border-2 border-amber-400 bg-amber-50/30 ring-1 ring-amber-200/50');
  }
  if (v.required) {
    return v.satisfied
      ? cn(base, 'border-2 border-emerald-500 bg-emerald-50/25 ring-1 ring-emerald-200/40')
      : cn(base, 'border-2 border-amber-300 bg-amber-50/25 ring-1 ring-amber-200/40');
  }
  return v.satisfied
